...
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im93dWxncGR1a3VlcWR1dmx1cmtzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE5NTE1OTUsImV4cCI6MjA4NzUyNzU5NX0.kt3x5bZcUNiMiG4fhQV13p2QaxfGAyR5c5taYorputA";
...
async function register() {
    ...
    const { data, error } = await supabase.from("users").insert([{ email, password, btc_balance: 0, loan: 0, transactions: [] }]);
...
async function loadDashboard() {
    ...
    if (document.getElementById("btcBalance")) document.getElementById("btcBalance").innerText = btcBalance.toFixed(6) + " BTC";
    if (document.getElementById("activeLoan")) document.getElementById("activeLoan").innerText = "$" + loan.toFixed(2);
...
async function loadTransactions() {
    const email = localStorage.getItem("currentUser");
    const { data: user } = await supabase.from("users").select("transactions").eq("email", email).single();
    ...
    transactions.forEach(tx => {
        container.innerHTML += `<div class="premium-card"><p>${tx.date} - ${tx.type} - ${tx.amount}</p></div>`;
    });
}
...
async function deposit() {
    ...
    btcBalance += input;
    const transactions = user.transactions || [];
    transactions.push({ type: "Deposit", amount: input + " BTC", date: new Date().toLocaleString() });

    const { error: updateErr } = await supabase.from("users").update({ btc_balance: btcBalance, transactions }).eq("email", email);
...
async function confirmLoan() {
    ...
    const transactions = user.transactions || [];
    transactions.push({ type: "Loan Borrowed", amount: requestedLoan + " USD", date: new Date().toLocaleString() });

    const { error: updateErr } = await supabase.from("users").update({ loan, transactions }).eq("email", email);
...
async function repayLoan() {
    ...
    loan -= repay;
    const transactions = user.transactions || [];
    transactions.push({ type: "Loan Repaid", amount: repay + " USD", date: new Date().toLocaleString() });
